...
- **TypeScript** — типизированный язык программирования, надмножество JavaScript, обеспечивающее статическую типизацию и улучшенную поддержку IDE.
- **React** — библиотека для построения пользовательских интерфейсов, основанная на компонентном подходе и виртуальном DOM.
- **Vite** — современный сборщик и dev-сервер, обеспечивающий быструю разработку и оптимизированную сборку проекта.
- **React Router** — библиотека для маршрутизации (роутинга) в React-приложениях, позволяющая реализовать SPA (Single Page Application).
- **Tailwind CSS** — утилитарный CSS-фреймворк, позволяющий быстро стилизовать компоненты с помощью готовых классов.
- **Axios** — асинхронный HTTP-клиент для выполнения запросов к API (бэкенду).
- **React Query (TanStack Query)** — библиотека для управления состоянием, кэширования и синхронизации данных, полученных с сервера.
- **Zustand** — легковесная библиотека для управления глобальным состоянием (state management) приложения.
- **React Hook Form** — библиотека для работы с формами, обеспечивающая валидацию, обработку ошибок и оптимизацию производительности.
- **i18next** — библиотека для интернационализации (i18n) и локализации (l10n) приложения, позволяющая переключать язык интерфейса.
- **date-fns** — утилитарная библиотека для работы с датами и временем (форматирование, парсинг, манипуляции).
- **React Icons** — набор готовых иконок (например, из Font Awesome, Material Design и т.д.) для использования в компонентах.
- **ESLint** — линтер для статического анализа кода, помогающий выявлять ошибки и поддерживать единый стиль кодирования.
- **Prettier** — инструмент для автоматического форматирования кода, обеспечивающий согласованный стиль (например, отступы, кавычки и т.д.).
- **Vitest** (или Jest) — фреймворк для модульного и интеграционного тестирования, позволяющий писать тесты для компонентов и логики.
- **Vercel** (или Netlify, GitHub Pages) — платформы для деплоя и хостинга статических файлов (собранного фронтенда), часто с интеграцией CI/CD.
